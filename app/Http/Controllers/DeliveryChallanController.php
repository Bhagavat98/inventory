...
{     
  ...
  */
  ...
  public function sendChallanEmail(Request $request){
        ...
        if (empty($to_cc_email)) {
          $to_cc_email = 'info@cosmicagps.com';
        }
        ...
        try{
            Mail::send('emails.challanMail', $data, function($message)use($pdf,$data,$to,$to_name,$challan_no,$to_cc_email) {
            $message->to($to, $to_name)
            ->cc($to, $to_name)
            //if(!empty($to_cc_email) ){
            ->cc([$to_cc_email]) 
            //}
            ->subject('Delivery Challan No.'.$challan_no.' from COSMICA TELEMATICS PVT LTD')
  ...
  *  challanMail
  */
  public function challanMail(Request $request){

    return view('emails.challanMail');
  }
