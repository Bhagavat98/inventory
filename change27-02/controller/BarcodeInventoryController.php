...
{     
  ...
  */
  public function show(){
    ...
    //echo json_encode($allDeviceArr);die();
    //dd($allDeviceArr);
  ...
  */
  public function showFromDate(Request $request){
    ...
    foreach ($barcodeimeiDisplay as $value) {
        ...
        // check device table
        $checkDevice = DB::table('device')
        ->where('imei',$value->imei)
        ->first();

        // check sim table
        $checkSim = DB::table('sim')
        ->where('sim_no',$value->imei)
        ->first();
    ...
    // imei list
    $barcodeimei = DB::table('advgps.barcode_imei_inventory')
                  ->get();

    foreach ($barcodeimei as $value) {
    ...
    if (!empty($barcodeimeiDisplay)) { // if not empty
      ...
      return view('showBarcodeInventory',['barcodeimei'=>$barcodeimei,'barcodeimeiDisplay'=>$barcodeimeiDisplay,'customerList'=>$customerList,'userList'=>$userList,'filterBy'=>$filterBy]);

    }else{

      return view('showBarcodeInventory',['barcodeimei'=>$barcodeimei,'barcodeimeiDisplay'=>$barcodeimeiDisplay,'customerList'=>$customerList,'userList'=>$userList,'filterBy'=>$filterBy])->with('error',''.$filterByName.'  Not Found.  please try Other.');
    }
